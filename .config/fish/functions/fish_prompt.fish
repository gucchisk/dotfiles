...
function __bobthefish_finish_segments -S -d 'Close open prompt segments'
    ...
    if [ "$theme_newline_cursor" = 'yes' ]
        ...
        # Display exit status indicator if last command failed
        if test -n "$last_status" -a "$last_status" -ne 0
            set_color -b $fish_color_error normal
            # echo -n " ✕ ($__bobthefish_last_status)"
            echo -n " "\uf05c" $last_status "
            set_color -b normal $fish_color_error
        else
            # set_color -b $fish_color_autosuggestion $fish_color_error
            set_color -b $color_path # 333333
            set_color $fish_color_param
            echo -n " "\uf05d" "
            # set_color -b normal $__bobthefish_current_bg
            set_color -b normal $color_path
        end
...
function __bobthefish_prompt_status -S -a last_status -d 'Display flags for a non-zero exit status, private mode, root user, and background jobs'
    ...
    if [ "$theme_newline_cursor" != 'yes' -a "$nonzero" -o "$fish_private_mode" -o "$superuser" -o "$bg_jobs" ]
